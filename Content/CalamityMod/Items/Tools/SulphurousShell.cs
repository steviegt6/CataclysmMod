...
namespace CataclysmMod.Content.CalamityMod.Items.Tools
{
    ...
    public class SulphurousShell : ModItem
    {
        ...
        {
            DisplayName.SetDefault("Sulphurous Shell"); // SHOULD be sulfurous but nah
            Tooltip.SetDefault("If you listen closely, you can hear your ear dissolving");
        }
        ...
        public override void HoldItem(Player player)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                Vector2 spawnPos = player.Bottom + Vector2.UnitY.RotatedBy(
                    player.itemAnimation * (Math.PI * 2f) / 30f
                ) * new Vector2(15f, 0f);
        ...
        private static void TeleportSulphurousShell(Player player)
        {
            ...
            if (specialTeleport)
                specialPos = landingPoint
                    .ToWorldCoordinates(8f, 16f) - new Vector2(player.width / 2f, player.height);
        ...
        private static bool RequestSulphurousTeleportPosition(
            Player player,
            int crawlOffsetX,
            int startX,
        ...
        )
        {
            ...
            while (loop1 < 10000 && loop2 < 10000)
            {
                ...
                if (IsInSolidTilesExtended(
                        new Vector2(point.X * 16 + 8, point.Y * 16 + 15) - halfPlayer,
                        player.velocity,
                        player.width,
                        player.height, (int) player.gravDir)
                   )
                {
                    if (tileIsSloped)
                        point.Y += 1;
                ...
                if (!IsInSolidTilesExtended(
                        new Vector2(point.X * 16 + 8, point.Y * 16 + 15 + 16) - halfPlayer,
                        player.velocity,
                        player.width,
                        player.height,
                        (int) player.gravDir
                    ) && !slopedOrLiquidBottom && point.Y < Main.worldSurface)
                {
                    point.Y += 1;
                    continue;
                }
